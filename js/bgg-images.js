/* LudoRef — BGG Image Loader v2
 * BGG IDs embarqués directement pour éviter une requête supplémentaire.
 * Charge les images depuis l'API BGG au runtime depuis le navigateur du visiteur.
 * Cache en localStorage 30 jours.
 */
(function() {
  'use strict';
  ...
  function setCache(slug, url) {
    try {
      localStorage.setItem(CACHE_PREFIX + slug, JSON.stringify({img: url, ts: Date.now()}));
    } catch(e) {}
  }

  function parseImg(xmlText) {
    try {
      const parser = new DOMParser();
      const doc = parser.parseFromString(xmlText, 'text/xml');
      // Essayer <image> d'abord, puis <thumbnail>
      const imgEl = doc.querySelector('image') || doc.querySelector('thumbnail');
      if (!imgEl || !imgEl.textContent.trim()) return null;
      let u = imgEl.textContent.trim();
  ...
  function showImg(container, url) {
    ...
    wrapper.style.cssText = 'position:relative;border-radius:12px;overflow:hidden;background:var(--surface-2,#1a1a2e);';
  ...
  function showPlaceholder(container) {
    const emoji = document.querySelector('.fiche-header-emoji');
    const em = emoji ? emoji.textContent.trim() : '🎲';
    container.innerHTML = '<div style="width:100%;aspect-ratio:1;max-height:200px;background:var(--surface-2,#1e1e3a);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:3.5rem;">' + em + '</div>';
  }

  async function loadImg(slug, bggId, container, attempt) {
    attempt = attempt || 1;
    if (attempt > 4) { showPlaceholder(container); return; }
    ...
    try {
      ...
      if (r.status === 202) {
        // BGG rate limit - retry
        setTimeout(() => loadImg(slug, bggId, container, attempt + 1), 3000 * attempt);
        return;
      }
      if (!r.ok) {
        showPlaceholder(container);
      ...
      console.debug('[LudoRef] BGG image error:', e.message);
      if (attempt < 3) {
        setTimeout(() => loadImg(slug, bggId, container, attempt + 1), 2000);
      } else {
        showPlaceholder(container);
      }
    }
